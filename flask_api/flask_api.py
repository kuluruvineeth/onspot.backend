...
# def hello():
#     return "hello vineeth you can conquer anything"
...
def upload_files():
    if not os.path.exists(localutility.IMG_UPLOADS_DIRECTORY):
        os.makedirs(localutility.IMG_UPLOADS_DIRECTORY)

    file_names = []
    for key in request.files:
        file = request.files[key]
        fn = str(random.getrandbits(128)) + localutility.DEFAULT_FILE_TYPE
        file_names.append(fn)
        try:
            file.save(os.path.join(localutility.IMG_UPLOADS_DIRECTORY, fn))
        except:
            print('save fail: ' + os.path.join(localutility.IMG_UPLOADS_DIRECTORY, fn))

    return dumps({'filename': [localutility.IMG_UPLOADS_DISPLAY_URL + f for f in file_names]})
...
if __name__ == '__main__':
    ...
    # app.run(host='0.0.0.0',ssl_context=('fullchain.pem','privkey.pem')) # for https in production
